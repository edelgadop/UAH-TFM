...
#----------------------------------------------------------------------------------------
# Una funcion para añadir dias al modelo que no debens ser usados
#----------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------
# de esta forma podriamos sacar los domingos y con la instrucción (comentada) añadirlo al estado
#----------------------------------------------------------------------------------------
domingos <- seq(as.Date("2016-01-01"),as.Date("2016-12-31"),by='day')
domingos <- domingos[weekdays(domingos)=='domingo']
#y <- AddHolidayEffect(y, domingos, c(.0, 10, .0))
#----------------------------------------------------------------------------------------
...
proceso_con_regresores <- function(element_key, mesHasta) {
  ...
  # la elección de las variables viene de usar la funcion     plot(modelo, "coefficients")
  # esto dibuja un grafico donde muestra las significativas
  if (mesHasta == 2)
  {
    plot(modelo, "coefficients")
  }
