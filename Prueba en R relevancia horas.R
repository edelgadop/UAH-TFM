...
#Voy a generar dos nuevos campos por franja horaria: 1.mañana y tarde(6h cada uno) 2.mañana, medio día y tarde(4h cada uno)
#Este campo nos puede resultar útil para reducir el número de registros en los modelos y que no pesen tanto

df_1$time_zone_1 <- as.factor(ifelse(df_1$hour < 14, 'mañana', 'tarde'))

df_1$time_zone_2 <- as.factor(ifelse(df_1$hour < 12, 'mañana', 
                                ifelse(df_1$hour > 16, 'tarde', 'medio_dia')))
...
relevancia(df_2$ocu, df_2$time_zone_1)

relevancia(df_2$ocu, df_2$time_zone_2)

relevancia(df_2$ocu, df_2$hour)

#Necesitamos encontrar un termino medio entre time_zone_2 y hour
