...
STdata_0 <- select(STdata, element_key, latitude, longitude, Month, Day, Hour, road_temp, monday, 
                   tuesday, wednesday, thursday, friday, tmin, day_year, occupation_perc)
ek_list <- unique(STdata_0$element_key)
ek_list <- ek_list[1:15]
STdata_final <- filter(STdata_0, (element_key %in% ek_list) & (Month <= 3))
day_list <- unique(STdata_final$day_year)

models = c("GP", "AR") #, "truncatedGP")
transf = c("NONE", "SQRT")
...
for (i in n:(length(day_list)-1)){
  ...
  for (mod in models){
    for (tr in transf){
#      if ((mod == "truncatedGP") & (tr == "SQRT")){break}
      if ((mod == "AR") & (tr == "SQRT")){break}
      ...
      res_row <- data.frame(model=mod, transf=tr, MAE=val_mae)
...
write.csv(resultados_final, file='results_sptimer_1t_alleks.csv', row.names=FALSE)
